...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    data_loss = np.mean(-scores_correct + np.log(np.sum(np.exp(scores),axis=1)))
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in xrange(1,self.num_layers):
      layer[i], cache[i] = affine_relu_forward(layer[i-1],self.params["W%d" % i],self.params["b%d" % i])
      
    
    scores, cache_scores = affine_forward(layer[self.num_layers-1],self.params["W%d" % (i+1)],self.params["b%d" % (i+1)])
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    for i in xrange(1,self.num_layers+1):
      reg_loss_data = 0.5*self.reg*np.sum(self.params["W%d" % i]*self.params["W%d" % i])
      reg_loss = reg_loss + reg_loss_data
