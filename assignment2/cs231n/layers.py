...
def affine_forward(x, w, b):
  ...
  #x = x.reshape(x.shape[0], np.product(input_shape))
  out = x.reshape(x.shape[0], np.product(input_shape)).dot(w)+b
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b)
...
def affine_backward(dout, cache):
  ...
  dw = np.dot(x.reshape(x.shape[0], np.product(input_shape)).T,dout)
        ...
        for j in xrange(F):
            for k in xrange(H_new):
                ...
                for l in xrange(W_new):
                    ...
                    window = x_pad[i, :, hs:hs+HH, ws:ws+WW]

                    #out[i, j, k, l] = np.sum(window*w[j]) + b[j]
                    dx_pad[i,:,hs:hs+HH, ws:ws+WW] += w[j] * dout[i,j,k,l]
                    db[j]+= dout[i,j,k,l]
