...
def sgd_nest_momentum(x, dx, config=None):
  """
  Performs stochastic gradient descent with momentum.
  ...
  v = config["momentum"] * v - config["learning_rate"] * dx # velocity update stays the same
  x += -config["momentum"] * v_prev + (1 + config["momentum"]) * v # position update changes form
  next_x = x
